...
def detect_feg_signal(
    ...
    Wick filter (optional, per-direction):
      c2_buy_upper_wick_max_pct:  BUY — ngưỡng % body C2 cho râu trên (high-close). None = tắt.
      c2_buy_lower_wick_max_pct:  BUY — ngưỡng % body C2 cho râu dưới (open-low). None = tắt.
      c2_sell_upper_wick_max_pct: SELL — ngưỡng % body C2 cho râu trên (high-open). None = tắt.
      c2_sell_lower_wick_max_pct: SELL — ngưỡng % body C2 cho râu dưới (close-low). None = tắt.

      c2_*_wick_cmp: hướng so sánh cho từng râu — "lt" (nhỏ hơn, default) | "gt" (lớn hơn).
        "lt" → reject khi râu >= pct% body  (râu quá dài)
        "gt" → reject khi râu <= pct% body  (râu quá ngắn)
    """
