...
BotForex là ứng dụng Streamlit đa trang để quản lý, backtest và vận hành bot giao dịch MT5 tự động. Hỗ trợ hai chiến lược: **Master Candle** (vào lệnh theo giờ cố định) và **FEG EMA21** (quét pattern 2 nến liên tục với bộ lọc EMA21, cả 2 nến phải cùng hướng). Layout compacted 2 cột với colored section headers (indigo/emerald/amber/red). Deploy tự động qua GitHub Actions + Tailscale SSH. Trace ID system cho mỗi backtest run và live order để debug dễ dàng.
...
- **Dashboard Streamlit** đa trang: Bots, Orders, Signals, Strategies, Backtest, Settings
  - Layout 2 cột compacted với colored section headers
  - Entry zone: FEG Margins + Wick Filter (left) | EMA + Time Window + Entry Mode (right)
...
- **Vietnamese labels**: Giao diện được dịch hoàn toàn sang tiếng Việt
...
- **Layout redesign (v0.3.1)** — 2-column compacted form zones, colored headers, removed Classic layout variants
- **FEG wick formula fix (v0.3.1)** — SELL upper wick (h2-o2), BUY lower wick (o2-l2)
- **Deprecated API fix (v0.3.1)** — Replaced `use_container_width` → `width='stretch'|'content'`
- **Vietnamese UI** — Full translation with approved labels
